...
class XGB:
    ...
    def __init__(self, df, target, test_size, random_state, k_folds, n_estimators, eta):     # starting point: n_estimators defaul = 100; eta defaul = 0.3 (to get L shape increase n; to avoid long-tail, decrease eta)
        self.df = df
        self.target = target
    ...
    def Display_Model_Results(self):
        ...
        dict = {'Model Name': self.model_name,
                ...
                'Target': self.target,
                f'Mean Accuracy ({self.k_folds} folds)': mean_accuracy,
                'RMSE': rmse,
                'F1-score': f1,
                'ROC-AUC score': auc}
